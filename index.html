...
async def add_reason(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Any icks? Send them one by one, then tap Done.\nOr tap Skip.",
...
async def add_ick_item(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "Added! 🤢 Send another ick or tap Done.",
        reply_markup=ReplyKeyboardMarkup(
            [[KeyboardButton("✅ Done"), KeyboardButton("⏭️ Skip")]],
            resize_keyboard=True
        )
...
async def add_icks_done(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Got it! Now send a photo or screenshot.\nOr tap Skip.",
        reply_markup=ReplyKeyboardMarkup([[KeyboardButton("⏭️ Skip")]], resize_keyboard=True)
    )
    return PHOTO
...
async def skip_icks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    context.user_data["new_icks"] = []
    await update.message.reply_text(
        "Now send a photo or screenshot.\nOr tap Skip.",
        reply_markup=ReplyKeyboardMarkup([[KeyboardButton("⏭️ Skip")]], resize_keyboard=True)
    )
    return PHOTO
...
async def lookup_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"📅 Filed: {entry.get('date', 'unknown date')}\n\n"
        f"🚩 Why you said no:\n{entry['reason']}"
        f"{ick_text}"
    )
...
async def edit_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not entries:
        ...
        return ConversationHandler.END
    keyboard = [
        [InlineKeyboardButton(entry["name"], callback_data=f"edit_who:{key}")]
        for key, entry in entries.items()
    ]
    await update.message.reply_text("Who do you want to edit?", reply_markup=InlineKeyboardMarkup(keyboard))
    return EDIT_SELECT

async def edit_who_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    key = query.data.replace("edit_who:", "")
    ...
    keyboard = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("🚩 Reason", callback_data="edit_field:reason"),
         InlineKeyboardButton("🤢 Icks", callback_data="edit_field:icks")],
        [InlineKeyboardButton("📸 Photo", callback_data="edit_field:photo")],
    ])
...
async def edit_field_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field == "icks":
        ...
        current_icks = entry.get("icks", [])
        ick_list = "\n".join(f"  — {ick}" for ick in current_icks) if current_icks else "  (none yet)"
        await query.edit_message_text(
            f"Current icks:\n{ick_list}\n\n"
            "Send the full new icks list, one per line.\n"
            "Or send a single dash — to clear all icks."
        )
    elif field == "photo":
        await query.edit_message_text("Send a new photo, or type /removephoto to remove the current one.")
        return EDIT_PHOTO
    else:
        labels = {"name": "name", "description": "description", "reason": "reason"}
        await query.edit_message_text(f"Send the new {labels[field]}:")
...
async def edit_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    entry = data[user_id][key]

    if field == "name":
        old_key = key
        new_key = new_value.lower()
        entry["name"] = new_value
        data[user_id][new_key] = entry
        del data[user_id][old_key]
    elif field == "icks":
        if new_value == "-":
            entry["icks"] = []
        else:
            entry["icks"] = [line.strip() for line in new_value.split("\n") if line.strip()]
    else:
        entry[field] = new_value

    save_data(data)
    await update.message.reply_text(f"✅ Updated successfully!", reply_markup=MAIN_KEYBOARD)
    return ConversationHandler.END
...
async def edit_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in data and key in data[user_id]:
        data[user_id][key]["photo"] = update.message.photo[-1].file_id
        save_data(data)
        await update.message.reply_text("✅ Photo updated!", reply_markup=MAIN_KEYBOARD)
    return ConversationHandler.END
...
async def remove_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = load_data()
    if user_id in data and key in data[user_id]:
        data[user_id][key]["photo"] = None
        save_data(data)
        await update.message.reply_text("✅ Photo removed.", reply_markup=MAIN_KEYBOARD)
    return ConversationHandler.END
...
async def addick_name_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    context.user_data["ick_target"] = query.data.replace("addick:", "")
...
async def addick_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = load_data()
    if user_id in data and key in data[user_id]:
        data[user_id][key].setdefault("icks", []).append(update.message.text.strip())
        save_data(data)
        await update.message.reply_text(f"🤢 Ick added to {data[user_id][key]['name']}'s file!", reply_markup=MAIN_KEYBOARD)
    else:
        await update.message.reply_text("Couldn't find that entry.", reply_markup=MAIN_KEYBOARD)
    return ConversationHandler.END
...
def main():
    ...
    edit_conv = ConversationHandler(
        entry_points=[
            ...
            MessageHandler(filters.Regex("^✏️ Edit$"), edit_start),
        ],
        states={
            EDIT_SELECT: [CallbackQueryHandler(edit_who_callback, pattern="^edit_who:")],
            EDIT_FIELD: [CallbackQueryHandler(edit_field_callback, pattern="^edit_field:")],
